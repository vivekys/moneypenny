...
class YahooListOfScripsFetcher {
  ...
  def getNextUrlTemplate (url : String) = {
    ...
    try {
      ...
      case ex : Exception => logger.error(s"Error while running getNextUrlTemplate for $url", ex)
        throw ex
    }
  }
  ...
  def getScrips (url : String) = {
    ...
    try {
      ...
      case ex : Exception => logger.error(s"Error while running YahooListOfScripsFetcher for $url", ex)
        throw ex
    }
  }
